...
授权后可使用 `/help` 查看普通指令；管理员会额外看到管理指令。直接发送贴纸后，
Bot 会先确认收到并持续显示“正在输入”状态，再自动识别类型并回传 PNG 或 GIF
文件。除 `/start <邀请码>` 外，其他功能只允许已授权用户和管理员使用。

## 测试镜像

在有 Docker 的环境中构建并运行 Python 与媒体转换测试：
...
在 Debian、Ubuntu 或其他 Linux 环境中运行一键安装脚本测试：
